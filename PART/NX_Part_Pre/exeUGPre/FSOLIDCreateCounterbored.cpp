...
int FSOLIDCreateCounterbored::_cntbrdCnt = 1;

FSOLIDCreateCounterbored::FSOLIDCreateCounterbored(Part * part, NXOpen::Features::Feature* ugfeature, NXOpen::Features::FeatureCollection * featureList)
	: Feature(part,ugfeature, featureList)
{
	_fType = SOLID_Create_Counterbored;
}

FSOLIDCreateCounterbored::~FSOLIDCreateCounterbored(void)
{
	
}


void FSOLIDCreateCounterbored::GetUGInfo()
{
	cout << "NX Feature Name: " << _ugFeature->GetFeatureName().GetUTF8Text() << endl;
	cout << "Get FSOLIDCreateCounterbored Info" << endl;
	...
	*/	
	...
	head_depth = ValRnd(holePackBuilder->GeneralCounterboreDepth()->Value());
	head_diameter = ValRnd(holePackBuilder->GeneralCounterboreDiameter()->Value());
	depth = ValRnd(holePackBuilder->DrillSizeHoleDepth()->Value());
	bottom_angle = ValRnd(holePackBuilder->DrillSizeTipAngle()->Value());
	lower_hole_diameter = ValRnd(holePackBuilder->DrillSizeHoleDiameter()->Value());
	
	cout << "Head Depth: " << head_depth << endl;
	cout << "Head Diameter: " << head_diameter << endl;
	cout << "Depth: " << depth << endl;
	cout << "Lower Diameter: " << lower_hole_diameter << endl;
...
void FSOLIDCreateCounterbored::ToTransCAD()
{
	...
	for (int i=0; i<points.size(); i++){
		...
		char buffer[20];
		_itoa( _cntbrdCnt++, buffer, 10 );
		SetName("CounterboredHole" + (string)buffer);
		GetPart()->_spFeatures->AddNewSolidHoleCounterboredFeature(GetName().c_str(), spSelectedPlane, points.at(i).X, points.at(i).Y, points.at(i).Z, head_diameter/2, head_depth, lower_hole_diameter/2, depth, bottom_angle);
		cout << "FSOLIDCreateCounterbored: " << GetName().c_str() << " Translated" << endl;
	}
	cout << endl;
}