...
void FeatureSOLIDCreateCutRevolve::GetInfo()
{
	...
	cout << " Cut revolve startAngle-> " << _startAngle << endl; //80
	cout << " Cut revolve endAngle-> " << _endAngle << endl;

	TransCAD::IReferencePtr spReference = spFeature->ProfileSketch;
	string sketchName = spReference->ReferenceeName;
	cout << " Cut revolve sketch-> " << sketchName.c_str() <<endl;

	cout << " Cut revolve Angle : " << (abs(_angle)/180*PI) << endl;
...
void FeatureSOLIDCreateCutRevolve::ToUG()
{
	try
	{
		...
		{
			Session::UndoMarkId cutRevolveCreateUndo;
			cutRevolveCreateUndo = _Part->_nxSession->SetUndoMark(Session::MarkVisibilityVisible, "cutRevolveCreateUndo");
			...
			/**
			 * Target body를 탐색
			 * target body가 존재하는 경우 -> tool body와 구분하고 Subtract 작업을 수행
			...
			**/
			...
			{		
				...
				{
					_Part->_nxSession->UndoToMark(cutRevolveCreateUndo, "cutRevolveCreateUndo");
					...
					cout << "Cut Revolve Feature JID: " << revolveFeat->JournalIdentifier().GetUTF8Text() << endl;
				}
			}
	...
	{
		cout << "   Error location [ Cut Revolve feature ]" << endl;
...
NXOpen::Features::RevolveBuilder * FeatureSOLIDCreateCutRevolve::revolveBuilderSet(FSketch * profileSketch, double sA, double eA)
{
	try
	{
		using namespace NXOpen;
...
BOOL FeatureSOLIDCreateCutRevolve::isIntersect(NXOpen::Body * toolBody, NXOpen::Body * targetBody)
{
	using namespace NXOpen;
