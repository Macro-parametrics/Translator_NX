...
FeatureSOLIDCreateCountersunk::FeatureSOLIDCreateCountersunk(Part * pPart, TransCAD::IFeaturePtr spFeature)
	: Feature(pPart, spFeature)
{
}

FeatureSOLIDCreateCountersunk::~FeatureSOLIDCreateCountersunk(void) {}

void FeatureSOLIDCreateCountersunk::GetInfo()
{
	...
	_sinkDiameter=2*(spFeature->GetSinkRadius());
	_bottomAngle=spFeature->GetBottomAngle();
	_diameter=2*(spFeature->GetRadius());
	_depth=spFeature->GetDepth();
	
	/*cout << "\n  FeatureSOLIDCreateCountersunk startDepth = " << _startDepth << endl;
	cout << "  FeatureSOLIDCreateCountersunk endDepth   = " << _endDepth << endl;
	cout << "FeatureSOLIDCreateCountersunk startDepth = " << _startDiameter << endl;
	cout << "  FeatureSOLIDCreateCountersunk endDepth   = " << _endDiameter << endl;*/
}

void FeatureSOLIDCreateCountersunk::ToUG()
{
	try
	{
		using namespace NXOpen;

		Features::Feature *cntsnk;
		...
		cntsnk=builder->CommitFeature();

		g_pRefManager->AddNewJID(_spFeature,cntsnk->JournalIdentifier().GetUTF8Text(),targetBodies[0]->JournalIdentifier().GetUTF8Text());
		cout<<"countersunk feature JID: "<<cntsnk->JournalIdentifier().GetUTF8Text()<<endl;

		builder->Destroy();
	}
	catch(const NXOpen::NXException &ex)
	{
		cout<<"Error location [countersunk]"<<endl;
		cout<<"Error code->"<<ex.ErrorCode()<<endl;
		cout<<"Error message->"<<ex.Message()<<endl;
	}
}

NXOpen::Features::HolePackageBuilder * FeatureSOLIDCreateCountersunk::HolePakageBuilderSet()
{
	try
	{
		using namespace NXOpen;
		...
		Point3d orgInSket(_center[0],_center[1],_center[2]);/////// 바꿔야 함
		...
		builder->SetGeneralHoleForm(Features::HolePackageBuilder::HoleFormsCountersink);
		
		builder->GeneralCountersinkAngle()->SetRightHandSide(to_string(_sinkAngle));
		builder->GeneralCountersinkDiameter()->SetRightHandSide(to_string(_sinkDiameter));
		builder->GeneralCountersinkHoleDiameter()->SetRightHandSide(to_string(_diameter));
		builder->GeneralCountersinkHoleDepth()->SetRightHandSide(to_string(_depth));
		//builder->GeneralTipAngle()->SetRightHandSide(to_string(_bottomAngle));
		builder->GeneralTipAngle()->SetRightHandSide("108");
...
string FeatureSOLIDCreateCountersunk::GetFaceJID(Point3D origin)
{
	using namespace NXOpen;
...
string FeatureSOLIDCreateCountersunk::GetBodyJID(Point3D origin)
{
	using namespace NXOpen;
